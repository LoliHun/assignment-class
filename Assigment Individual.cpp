...
{
	 int answer;
printf("Are you the parent? > "); scanf("%d", &answer);
printf("You are %d my parent", answer); 
 ...
 if (cpid == 0) 
  {                               /* Child reads from pipe */
   ...
   printf("The child is will read from the pipe.\n");
   while (read(pipefd[0], &buf, 1) > 0)
     write(STDOUT_FILENO, &buf, 1);
   write(STDOUT_FILENO, "\n", 1);
   close(pipefd[0]);
   printf("The child has just split from the pipe to standard output.\n");
   _exit(EXIT_SUCCESS);
  } 
 else 
